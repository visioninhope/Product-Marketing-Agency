...
class ProductProfile:
    ...
    objectives: List[str]
    suggested_image_types: List[int]  # Array of numbers matching ImageType enum values
    product_id: str = None
...
class ProductMarketingAgency:
    ...
    def _setup_logging(self):
        ...
        log_file = self.save_directory / "logs" / f"marketing_agency_{time.strftime('%Y%m%d')}.log"
    ...
    def _track_base64_globally(self, base64_data: str) -> bool:
        ...
        if 'data:image' in base64_content and 'base64,' in base64_content:
            # Extract just the base64 part for consistent hashing
            _, base64_content = base64_content.split('base64,', 1)
        
        base64_hash = hashlib.sha256(base64_content.encode()).hexdigest()
        ...
        if tracking_file.exists():
            try:
                ...
                self.logger.warning(f"Could not load base64 tracking file: {e}")
                tracked_hashes = {}
        ...
        if len(tracked_hashes) > 1000:
            ...
            sorted_hashes = sorted(tracked_hashes.items(), key=lambda x: x[1], reverse=True)
    ...
    def _convert_base64_to_file(self, base64_data: str, output_dir: Path, filename_prefix: str) -> Optional[str]:
        ...
        try:
            ...
            if '_' in filename_prefix and len(filename_prefix.split('_')) >= 4:
            ...
            self.logger.info(f"Base64 image converted and saved to: {output_path}")
            return str(output_path)
            
        except Exception as e:
            self.logger.error(f"Failed to convert base64 to image file: {str(e)}")
            return None

    def _get_intelligent_image_suggestions(self, product_data: Dict[str, Any]) -> List[int]:
        """
        ...
        all_text = f"{category} {name} {description} {' '.join(features)}"
        
        suggested_types = []
        
        # Electronics / Tech products
        if any(word in all_text for word in ['electronic', 'tech', 'smart', 'digital', 'wireless', 'bluetooth', 'app', 'battery', 'charging']):
            suggested_types.extend([1, 3, 7, 9])  # MASTER_PRODUCT_SHOT, EXTREME_MACRO_DETAIL, LIFESTYLE_ACTION_SHOT, NEGATIVE_SPACE_BANNER
        
        # Fashion / Wearable products
        elif any(word in all_text for word in ['fashion', 'clothing', 'shoe', 'sneaker', 'watch', 'jewelry', 'accessory', 'wearable']):
            suggested_types.extend([1, 4, 5, 7, 8])  # Add COLOR_VARIATIONS and ON_FOOT_SIZE_COMPARISONS
        
        # Food / Beverage products
        elif any(word in all_text for word in ['food', 'drink', 'beverage', 'nutrition', 'organic', 'healthy', 'recipe']):
            suggested_types.extend([1, 2, 8, 10])  # MASTER_PRODUCT_SHOT, WHATS_IN_THE_BOX, UGC_STYLE, SHOP_THE_LOOK
        
        # Home / Office products
        elif any(word in all_text for word in ['home', 'office', 'desk', 'furniture', 'kitchen', 'decor', 'organization']):
            suggested_types.extend([1, 6, 7, 9])  # Include MODEL_COMPOSITE for lifestyle context
        
        # Fitness / Sports products
        elif any(word in all_text for word in ['fitness', 'sport', 'exercise', 'workout', 'athletic', 'training', 'gym']):
            suggested_types.extend([1, 5, 7, 8])  # Focus on action and size comparisons
        
        # Beauty / Personal Care
        elif any(word in all_text for word in ['beauty', 'cosmetic', 'skincare', 'personal', 'care', 'makeup', 'fragrance']):
            suggested_types.extend([1, 3, 4, 8])  # Detail shots and variations important
        
        # Automotive / Transportation
        elif any(word in all_text for word in ['car', 'automotive', 'vehicle', 'bike', 'transport', 'motor']):
            suggested_types.extend([1, 3, 6, 7])  # Product shots, details, composites, action
        
        # Default fallback for unknown categories
        if not suggested_types:
            suggested_types = [1, 7, 9]  # Conservative default: MASTER_PRODUCT_SHOT, LIFESTYLE_ACTION_SHOT, NEGATIVE_SPACE_BANNER
        
        # Remove duplicates and limit to 4-6 suggestions
        unique_suggestions = list(dict.fromkeys(suggested_types))  # Preserves order, removes duplicates
        
        # Limit to 4-6 suggestions for better user experience
        final_suggestions = unique_suggestions[:6] if len(unique_suggestions) > 6 else unique_suggestions
        
        self.logger.info(f"Generated intelligent suggestions for category '{category}': {final_suggestions}")
        return final_suggestions

    def _get_image_type_specific_params(self, product_profile: ProductProfile, image_type: ImageType) -> Dict[str, str]:
        """
        ...
        base_params = {
            ...
            "product_description": f"{product_profile.product_name} - {product_profile.category}",
        ...
        if image_type == ImageType.MASTER_PRODUCT_SHOT:
            ...
            backgrounds = ["pristine white cyclorama", "gradient infinity cove", "brushed aluminum surface", "matte black studio"]
            lighting_setups = ["three-point studio lighting with softboxes", "rim lighting with key light", "butterfly lighting setup", "split lighting with reflectors"]
            angles = ["elevated 45-degree hero", "straight-on eye level", "low angle dramatic", "three-quarter view"]
            atmospheres = ["luxury retail showroom", "high-end gallery space", "premium boutique setting", "flagship store display"]
            
            return {
                **base_params,
                "background_surface": random.choice(backgrounds),
                "lighting_setup": random.choice(lighting_setups),
                "lighting_purpose": "sculpting dimension and highlighting premium materials",
                "angle_type": random.choice(angles),
                "specific_feature": ", ".join(product_profile.key_features[:2]) if product_profile.key_features else "premium build quality",
                "key_detail": product_profile.key_features[0] if product_profile.key_features else "signature design elements",
                "atmosphere_description": random.choice(atmospheres),
                "style_reference": random.choice(["Apple product photography", "Leica camera aesthetics", "Bang & Olufsen style"]),
                "camera_equipment": random.choice(["Hasselblad H6D-100c", "Phase One XF IQ4", "Canon 5DS R with 100mm macro"]),
                "post_production": random.choice(["subtle vignetting", "micro-contrast enhancement", "color grading for depth"]),
                "negative_prompt": "Avoid: blurry edges, cartoonish rendering, flat lighting, cluttered background, amateur photography, stock photo look, oversaturated colors, visible dust or artifacts",
            }
            
        elif image_type == ImageType.LIFESTYLE_ACTION_SHOT:
            activities = ["urban street performance", "outdoor adventure expedition", "intense workout session", "creative collaboration", "competitive sports moment"]
            environments = ["bustling city streets", "mountain trail at sunrise", "industrial urban landscape", "beachfront at sunset", "dynamic studio space"]
            techniques = ["motion blur with sharp subject", "freeze-frame action", "panning shot technique", "high-speed capture", "dynamic composition"]
            moods = ["energetic and vibrant", "aspirational and bold", "raw and authentic", "inspiring and uplifting"]
            
            return {
                **base_params,
                "activity_context": random.choice(activities),
                "model_demographic": random.choice(["professional athletes", "creative professionals", "adventure enthusiasts", "urban explorers"]),
                "action_verb": random.choice(["performing", "competing", "creating", "exploring", "challenging limits with"]),
                "camera_technique": random.choice(techniques),
                "environment_description": random.choice(environments),
                "lighting_condition": random.choice(["golden hour", "blue hour", "dramatic storm light", "high-contrast midday"]),
                "mood_atmosphere": random.choice(moods),
                "focus_technique": random.choice(["selective focus", "zone focusing", "continuous AF tracking"]),
                "focal_point": "the product in active use",
                "color_palette": random.choice(["vibrant and energetic", "moody and dramatic", "natural and earthy"]),
                "photography_style": random.choice(["documentary", "editorial", "cinematic", "reportage"]),
                "negative_prompt": "Avoid: static poses, studio lighting, plain backgrounds, stock photo appearance, unnatural movements, overprocessed look, fake motion blur",
            }
            
        elif image_type == ImageType.NEGATIVE_SPACE_BANNER:
            compositions = ["rule of thirds", "golden ratio", "diagonal balance", "asymmetric harmony"]
            color_schemes = ["monochromatic minimal", "dual-tone contrast", "gradient subtle", "pure white space"]
            placements = ["lower left third", "upper right corner", "centered bottom", "offset diagonal"]
            
            return {
                **base_params,
                "composition_rule": random.choice(compositions),
                "color_scheme": random.choice(color_schemes),
                "space_percentage": random.choice(["20%", "25%", "30%"]),
                "product_position": random.choice(placements),
                "text_space_location": "strategic text placement areas",
                "brand_aesthetic": random.choice(["Swiss minimalism", "Scandinavian design", "Japanese zen aesthetics"]),
                "shadow_treatment": random.choice(["soft drop shadow", "no shadow pure float", "subtle ground shadow"]),
                "design_principle": random.choice(["less is more", "form follows function", "negative space as design element"]),
                "negative_prompt": "Avoid: cluttered composition, busy background, multiple products, decorative elements, text overlays, logos, complex patterns",
            }
            
        elif image_type == ImageType.WHATS_IN_THE_BOX_FLAT_LAY:
            arrangements = ["geometric grid", "organic flow", "radial burst", "diagonal cascade", "symmetrical balance"]
            surfaces = ["marble texture", "wood grain surface", "concrete industrial", "pristine white", "dark matte backdrop"]
            lighting = ["soft overhead diffusion", "directional window light", "even studio lighting", "subtle gradient lighting"]
            moods = ["premium unboxing experience", "minimalist presentation", "luxurious reveal", "organized perfection"]
            
            return {
                **base_params,
                "item_1": product_profile.accessories[0] if len(product_profile.accessories) > 0 else "instruction manual",
                "item_2": product_profile.accessories[1] if len(product_profile.accessories) > 1 else "warranty card", 
                "item_3": product_profile.accessories[2] if len(product_profile.accessories) > 2 else "premium cable",
                ...
                "shadow_style": random.choice(["soft natural shadows", "minimal shadow presence", "dramatic shadow play"]),
                "negative_prompt": "Avoid: messy arrangement, overlapping items, perspective distortion, harsh shadows, cluttered composition, random placement",
            }
            
        elif image_type == ImageType.UGC_STYLE_PHOTOS:
            contexts = ["coffee shop moment", "home office setup", "weekend adventure", "daily commute", "friend gathering"]
            filters = ["VSCO filter", "Instagram Valencia", "natural no filter", "slight fade effect", "warm vintage tone"]
            platforms = ["Instagram feed", "TikTok video still", "Twitter post", "Pinterest save", "Snapchat story"]
            
            return {
                **base_params,
                "phone_camera_style": random.choice(["iPhone portrait mode", "Android wide angle", "front camera selfie style", "quick snapshot"]),
                "natural_lighting": random.choice(["window light", "outdoor shade", "indoor ambient", "mixed lighting"]),
                ...
                "aspect_ratio": random.choice(["1:1 square", "4:5 portrait", "9:16 stories", "16:9 landscape"]),
                "negative_prompt": "Avoid: professional photography, studio setup, perfect composition, commercial polish, stock photo look, staged scenarios",
            }
            
        elif image_type == ImageType.EXTREME_MACRO_DETAIL:
            technical_specs = ["100mm macro lens at 1:1", "focus stacking technique", "extension tubes setup", "microscope objective lens"]
            styles = ["scientific documentation", "artistic abstract", "technical precision", "luxury detail showcase"]
            
            return {
                **base_params,
                "detail_feature": product_profile.key_features[0] if product_profile.key_features else "signature texture",
                "texture_material": random.choice(["woven carbon fiber", "brushed metal grain", "leather texture", "technical fabric weave"]),
                "quality_aspect": random.choice(["handcrafted precision", "engineering excellence", "material innovation", "artisan craftsmanship"]),
                "focus_point": "microscopic detail revelation",
                "technical_description": random.choice(technical_specs),
                "lens_type": random.choice(["100mm macro", "180mm macro", "MP-E 65mm", "90mm macro"]),
                "dof_description": "razor-thin depth of field with dreamy bokeh",
                "style_description": random.choice(styles),
                "negative_prompt": "Avoid: full product view, wide angle, deep depth of field, context shots, lifestyle elements, multiple focus points",
            }
            
        elif image_type == ImageType.COLOR_STYLE_VARIATIONS:
            layouts = ["clean grid matrix", "diagonal lineup", "circular arrangement", "stepped cascade", "horizontal parade"]
            backgrounds = ["gradient backdrop", "color-matched backgrounds", "neutral gray studio", "white infinity", "complementary color blocks"]
            
            return {
                **base_params,
                "variation_count": random.choice(["3", "4", "5", "6"]),
                "variation_type": random.choice(["colorways", "material finishes", "style options", "seasonal variants"]),
                "layout_style": random.choice(layouts),
                "arrangement_description": "systematic progression showing range",
                "background_type": random.choice(backgrounds),
                "lighting_consistency": "uniform lighting across all variants",
                "photography_style": random.choice(["catalog precision", "editorial layout", "e-commerce standard"]),
                "composition_rule": "balanced visual weight distribution",
                "negative_prompt": "Avoid: inconsistent angles, varying scales, misaligned products, color cast differences, shadowing variations",
            }
            
        elif image_type == ImageType.ON_FOOT_SIZE_COMPARISONS:
            comparison_types = ["side-by-side comparison", "overlay demonstration", "progression sequence", "reference grid"]
            perspectives = ["eye level straight on", "45-degree angle", "top-down view", "profile silhouette"]
            
            return {
                **base_params,
                "comparison_type": random.choice(comparison_types),
                "comparison_objects": random.choice(["standard references", "familiar objects", "measurement tools", "hand for scale"]),
                "scale_demonstration": "clear size relationships",
                "measurement_indication": random.choice(["ruler visible", "grid background", "size labels", "dimensional callouts"]),
                ...
                "style_descriptor": random.choice(["technical documentation", "educational reference", "comparison chart style"]),
                "negative_prompt": "Avoid: perspective distortion, misleading scales, cluttered comparisons, inconsistent positioning",
            }
            
        elif image_type == ImageType.ADD_A_MODEL_TWO_IMAGE_COMPOSITE:
            composite_styles = ["split-screen diptych", "before/after panels", "layered double exposure", "side-by-side frames"]
            settings = ["modern office", "urban street", "home interior", "outdoor adventure", "creative studio"]
            moods = ["warm and inviting", "cool and professional", "energetic and dynamic", "calm and sophisticated"]
            
            return {
                **base_params,
                "composite_style": random.choice(composite_styles),
                "first_composition": "product in pristine studio setting",
                "model_demographic": random.choice(["young professionals", "creative millennials", "active lifestyle enthusiasts", "sophisticated urbanites"]),
                "environment_setting": random.choice(settings),
                "lighting_mood": random.choice(moods),
                "atmosphere_description": "cohesive narrative across both frames",
                "color_grading": random.choice(["consistent warm tones", "complementary color harmony", "monochromatic elegance"]),
                "negative_prompt": "Avoid: mismatched styles between frames, inconsistent lighting, different quality levels, jarring transitions",
            }
            
        elif image_type == ImageType.SHOP_THE_LOOK_FLAT_LAY:
            surfaces = ["marble surface", "rustic wood", "linen fabric", "concrete texture", "velvet backdrop"]
            principles = ["color coordination", "theme cohesion", "style matching", "seasonal curation", "trend alignment"]
            magazines = ["Vogue editorial", "Kinfolk minimal", "Cereal magazine", "Monocle style", "The Gentlewoman"]
            
            return {
                **base_params,
                "accessory_1": product_profile.accessories[0] if len(product_profile.accessories) > 0 else "style complement",
                "accessory_2": product_profile.accessories[1] if len(product_profile.accessories) > 1 else "accent piece",
                "accessory_3": product_profile.accessories[2] if len(product_profile.accessories) > 2 else "finishing touch",
                "surface_type": random.choice(surfaces),
                "styling_principle": random.choice(principles),
                "overhead_lighting": random.choice(["soft diffused daylight", "dramatic side light", "even flat lighting"]),
                "shadow_depth": random.choice(["subtle depth", "dramatic shadows", "minimal shadow"]),
                "color_coordination": "harmonious palette relationship",
                "spacing_rhythm": "balanced negative space distribution",
                "lifestyle_props": "curated selection of complementary items",
                "magazine_style": random.choice(magazines),
                "negative_prompt": "Avoid: random placement, clashing styles, overcrowded composition, mismatched aesthetics, poor color harmony",
            }
    ...
    def create_product_profile(self, product_data: Dict[str, Any]) -> ProductProfile:
        """
        ...
        self.logger.info(f"Creating product profile for: {product_data.get('name', 'Unknown Product')}")
        
        try:
            # Run the agent to analyze the product
            analysis_result = self.product_interpreter_agent.run(prompt)
            ...
            try:
                ...
                self.logger.warning("Failed to parse JSON from agent response, using fallback parsing")
                # Intelligent fallback that suggests image types based on product category and features
                suggested_types = self._get_intelligent_image_suggestions(product_data)
                parsed_json = {
                    "product_name": product_data.get('name', 'Unknown Product'),
                    "category": product_data.get('category', 'General'),
                    "key_features": product_data.get('features', ['Feature 1', 'Feature 2', 'Feature 3']),
                    "accessories": product_data.get('accessories', ['Standard accessories']),
                    "objectives": product_data.get('objectives', ['Increase brand awareness', 'Drive sales', 'Showcase features']),
            ...
            profile = ProductProfile(
                ...
                suggested_image_types=parsed_json['suggested_image_types']
            )
            ...
            self.logger.info(f"Product profile created: {profile.product_id}")
            return profile
            
        except Exception as e:
            self.logger.error(f"Error creating product profile: {str(e)}")
            raise

    def select_image_type(self, product_profile: ProductProfile, interactive: bool = True) -> ImageType:
        """
        ...
        if interactive:
            return self._interactive_image_selection(product_profile.suggested_image_types)
        ...
        try:
            ...
            try:
                ...
                if 1 <= type_number <= 10:
                    ...
                    self.logger.info(f"AI recommended image type: {selected_type}")
            ...
            if product_profile.suggested_image_types:
                fallback_type = ImageType(product_profile.suggested_image_types[0])
                self.logger.warning(f"Could not parse AI recommendation, using first suggested type: {fallback_type}")
                return fallback_type
            
            # Final fallback
            self.logger.warning("Could not parse AI recommendation, defaulting to MASTER_PRODUCT_SHOT")
            return ImageType.MASTER_PRODUCT_SHOT
            
        except Exception as e:
            self.logger.error(f"Error in image type selection: {str(e)}")
            return ImageType.MASTER_PRODUCT_SHOT

    def _interactive_image_selection(self, suggested_types: List[int] = None) -> ImageType:
        """Interactive console-based image type selection"""
        
        # Rich styled header
        title = Text("MARKETING IMAGE TYPE SELECTION", style="bold cyan")
        panel = Panel(
            Align.center(title),
            border_style="cyan",
            box=box.ROUNDED
        ...
        image_table = Table(title="[bold yellow]Available Marketing Image Types[/bold yellow]")
        ...
        image_options = [
            (1, ImageType.MASTER_PRODUCT_SHOT, "High-resolution studio product photography"),
            (2, ImageType.WHATS_IN_THE_BOX_FLAT_LAY, "Top-down arrangement with all included items"),
            (3, ImageType.EXTREME_MACRO_DETAIL, "Ultra close-up detail shots"),
            (4, ImageType.COLOR_STYLE_VARIATIONS, "Multiple color/style options display"),
            (5, ImageType.ON_FOOT_SIZE_COMPARISONS, "Size comparison photos with models"),
            (6, ImageType.ADD_A_MODEL_TWO_IMAGE_COMPOSITE, "Split image with and without model"),
            (7, ImageType.LIFESTYLE_ACTION_SHOT, "Dynamic action photography"),
            (8, ImageType.UGC_STYLE_PHOTOS, "Authentic user-generated content style"),
            (9, ImageType.NEGATIVE_SPACE_BANNER, "Product with space for text overlay"),
            (10, ImageType.SHOP_THE_LOOK_FLAT_LAY, "Complete styling flat lay arrangement")
        ]
        ...
        while True:
            try:
                choice = Prompt.ask("\n[bold cyan]Select image type[/bold cyan]", choices=[str(i) for i in range(1, 11)])
                ...
                console.print(f"[red]Invalid input. Please try again. ({str(e)})[/red]")
    ...
    def generate_custom_prompt(
        ...
        prompt_params = self._get_image_type_specific_params(product_profile, image_type)
        
        # Apply template with parameters
        try:
            customized_prompt = base_template.format(**prompt_params)
        except KeyError as e:
            self.logger.warning(f"Missing template parameter {e}, using base template")
        ...
        try:
            enhanced_prompt = self.prompt_generator_agent.run(enhancement_prompt)
            self.logger.info(f"Generated custom prompt for Step_{image_type.value}")
    ...
    def generate_marketing_image(
        ...
        try:
            ...
            timestamp = time.strftime("%Y%m%d_%H%M%S_%f")  # Include microseconds for higher uniqueness
            random_suffix = str(uuid.uuid4())[:12]  # Longer random suffix
            ...
            enhanced_prompt = f"GENERATE IMAGE: {custom_prompt}\n\n" \
            ...
            self.logger.info(f"Agent response length: {len(result_str)} characters")
            self.logger.info(f"Response starts with: {result_str[:100]}...")
            self.logger.info(f"Response ends with: ...{result_str[-100:]}")
            
            # Create organized directory structure for this product and image type
            product_dir = self.save_directory / "outputs" / f"{product_profile.product_name.replace(' ', '_')}"
            image_type_dir = product_dir / f"Step_{image_type.value}_{image_type.name.lower()}"
            ...
            if (result_str.endswith(('.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.tiff')) and 
                os.path.exists(result_str)):
                ...
                meaningful_path = image_type_dir / f"{meaningful_filename}{file_extension}"
                
                # Move the file to organized location
                shutil.move(str(saved_path), str(meaningful_path))
                self.logger.info(f"Image moved from {result_str} to organized location: {meaningful_path}")
                self.logger.info(f"Unique filename generated: {meaningful_filename}{file_extension}")
            ...
            # Second check: Does this look like base64 image data?
            elif 'data:image' in result_str and 'base64' in result_str:
                
                # Found base64 image data - extract it from the mixed text response
                self.logger.info("Detected base64 image data in text response, extracting and converting to file...")
                ...
                if match:
                    ...
                    if converted_image_path:
                        ...
                        self.logger.error("Failed to convert base64 data to image file")
                else:
                    self.logger.warning("Could not extract valid base64 data URI from response")
                    
            # Third check: Is this pure base64 data (without data URI prefix)?
            elif len(result_str) > 1000 and re.match(r'^[A-Za-z0-9+/]+=*$', result_str.replace('\n', '').replace(' ', '').strip()):
            ...
            self.logger.warning(f"Unexpected response format. Length: {len(result_str)} characters. First 200 chars: {result_str[:200]}")
            ...
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(f"Raw Agent Response (Debug):\n")
    ...
    def process_output_and_feedback(
        ...
        try:
            feedback_analysis = self.output_handler_agent.run(feedback_prompt)
            ...
            feedback_path = self.save_directory / "jobs" / f"{job.job_id}_feedback.json"
            with open(feedback_path, 'w', encoding='utf-8') as f:
                json.dump(result, f, indent=2)
            
            self.logger.info(f"Feedback analysis completed for job: {job.job_id}")
            return result
            
        except Exception as e:
            self.logger.error(f"Error processing output and feedback: {str(e)}")
            raise
    ...
    def run_marketing_campaign(
        ...
        try:
            ...
            if image_types is None:
                if interactive:
                    image_types = [self.select_image_type(product_profile, interactive=True)]
                    
                    # Ask if user wants to generate multiple types
                    while True:
                        more = input("\nWould you like to generate another image type? (y/n): ").strip().lower()
                        if more in ['y', 'yes']:
                            image_types.append(self.select_image_type(product_profile, interactive=True))
                        elif more in ['n', 'no']:
                            break
                        else:
                            print("Please enter 'y' for yes or 'n' for no.")
                else:
                    image_types = [self.select_image_type(product_profile, interactive=False)]
            
            print(f" Selected {len(image_types)} image types for generation")
            ...
            for i, image_type in enumerate(image_types, 1):
                ...
                generated_result = self.generate_marketing_image(
                    product_profile, image_type, custom_prompt, master_image_path
                )
                
                # Process output and feedback
                print("  � Processing results and generating feedback...")
                final_result = self.process_output_and_feedback(generated_result, job)
                
                results.append(final_result)
                print(f" {image_type.name.replace('_', ' ').title()} completed successfully")
            
            # Step 4: Campaign summary
            print(f"\n<� CAMPAIGN COMPLETED!")
            print(f"Generated {len(results)} marketing assets for {product_profile.product_name}")
            print(f"Results saved in: {self.save_directory}")
            
            return results
            
        except Exception as e:
            self.logger.error(f"Error in marketing campaign workflow: {str(e)}")
            raise
    ...
    def batch_process_products(
        ...
        self.logger.info(f"Starting batch processing for {len(products_data)} products")
        ...
        print(f"Total assets generated: {sum(len(results) for results in batch_results)}")
    ...
    def _save_product_profile(self, profile: ProductProfile):
        ...
        profile_path = self.save_directory / "products" / f"{profile.product_id}.json"
        with open(profile_path, 'w', encoding='utf-8') as f:
            json.dump(asdict(profile), f, indent=2)

    def load_product_profile(self, product_id: str) -> Optional[ProductProfile]:
        """Load product profile from disk"""
        profile_path = self.save_directory / "products" / f"{product_id}.json"
    ...
    def set_master_image(self, product_id: str, image_path: str):
        ...
        if os.path.exists(image_path):
            ...
            if product_id in self.product_profiles:
                self.product_profiles[product_id].master_image_path = str(master_copy_path)
                self._save_product_profile(self.product_profiles[product_id])
            
            self.logger.info(f"Master image set for product {product_id}: {master_copy_path}")
    ...
    def ensure_visual_consistency(self, product_profile: ProductProfile, image_type: ImageType) -> bool:
    ...
    def get_campaign_statistics(self) -> Dict[str, Any]:
        ...
        for job in self.active_jobs.values():
            image_type = f"Step_{job.image_type.value}"
            stats["image_types_generated"][image_type] = stats["image_types_generated"].get(image_type, 0) + 1
...
def display_welcome_banner():
    ...
    # Create feature list
    features_table = Table(show_header=False, show_edge=False, pad_edge=False, box=None)
...
Master Reference Image Path: /path/to/image.jpg"""
        ...
        if json_match:
            ...
            try:
                ...
                product_data = {
                    ...
                    'category': str(parsed_data.get('category', '')).strip(),
                    'description': str(parsed_data.get('description', '')).strip(),
                    'features': [str(f).strip() for f in parsed_data.get('features', []) if str(f).strip()],
                    'target_audience': str(parsed_data.get('target_audience', '')).strip(),
                    'brand_colors': [str(c).strip() for c in parsed_data.get('brand_colors', []) if str(c).strip()],
                    'price_range': str(parsed_data.get('price_range', '')).strip(),
                    'usp': [str(u).strip() for u in parsed_data.get('usp', []) if str(u).strip()],
                    'custom_requirements': str(parsed_data.get('custom_requirements', '')).strip(),
    ...
    if usps:
        usps_display = "\n".join([f"{i}. {usp}" for i, usp in enumerate(usps[:2], 1)])
        if len(usps) > 2:
    ...
    required_fields = ['name', 'category', 'description', 'target_audience']
...
def fallback_manual_parsing(text: str) -> Dict[str, Any]:
    ...
    if 'category' not in product_data:
        if any(word in text_lower for word in ['headphone', 'audio', 'speaker', 'earphone']):
            product_data['category'] = 'Electronics'
        elif any(word in text_lower for word in ['shoe', 'sneaker', 'boot', 'clothing']):
            product_data['category'] = 'Fashion'
        elif any(word in text_lower for word in ['food', 'drink', 'beverage']):
    ...
    product_data['features'] = [f.strip() for f in features if f.strip()]
    ...
    usp_section = re.search(r'unique selling points?[:\s]*(.*?)(?=\n\n|\nCustom|\nMaster|$)', text, re.IGNORECASE | re.DOTALL)
    if usp_section:
...
#         print("Please set GEMINI_API_KEY or OPENAI_API_KEY before running.")
...
#         menu_table = Table(title="[bold magenta]MAIN MENU[/bold magenta]")
...
#         choice = Prompt.ask("\n[bold cyan]Select option[/bold cyan]", choices=["1", "2", "3", "4", "5", "6", "7"])
...
#                 custom_req = product_data.pop('custom_requirements', '')
#                 master_img_path = product_data.pop('master_image_path', '')
...
#                         'description': 'Smart water bottle with temperature control',
#                         'features': ['Temperature control', 'App connectivity', 'Leak-proof'],
#                         'target_audience': 'Health-conscious professionals',
#                         'brand_colors': ['#22c55e', '#ffffff'],
#                         'price_range': '$49-79',
#                         'usp': ['24hr temperature retention', 'Smart hydration tracking']
#                     },
#                     {
#                         'name': 'PowerDesk Pro',
#                         'category': 'Office Equipment',
#                         'description': 'Height-adjustable standing desk with wireless charging',
#                         'features': ['Height adjustment', 'Wireless charging pad', 'Cable management'],
#                         'target_audience': 'Remote workers and professionals',
#                         'brand_colors': ['#1f2937', '#f59e0b'],
#                         'price_range': '$299-499',
#                         'usp': ['Wireless charging integration', 'Memory presets']
#                     }
#                 ]
...
#                 if profiles:
#                     profiles_text = f"Available Profiles: {', '.join(profiles)}"
#                     console.print(f"\n[cyan]{profiles_text}[/cyan]")
#                     profile_id = Prompt.ask("Enter profile ID to load")
...
#                 stats_table.add_row("Total Products", str(stats['total_products']))
#                 stats_table.add_row("Total Jobs", str(stats['total_jobs']))
#                 stats_table.add_row("Completed Jobs", str(stats['completed_jobs']))
#                 stats_table.add_row("Save Directory", stats['save_directory'])
                
#                 console.print("\n")
#                 console.print(stats_table)
                
#                 if stats['image_types_generated']:
#                     types_table = Table(title="[bold yellow]Image Types Generated[/bold yellow]")
#                     types_table.add_column("Type", style="magenta")
#                     types_table.add_column("Count", style="cyan")
                    
#                     for img_type, count in stats['image_types_generated'].items():
#                         types_table.add_row(img_type.replace('_', ' ').title(), str(count))
...
#                 print("\n= HELP & DOCUMENTATION")
#                 print("-"*40)
#                 print("This system creates marketing materials using 6 specialized AI agents:")
#                 print("1. Orchestrator Agent - Manages workflow")
#                 print("2. Product Interpreter Agent - Analyzes products") 
#                 print("3. Image Type Selector Agent - Guides selection")
#                 print("4. Prompt Generator Agent - Creates custom prompts")
...
#                 print("All generated content is saved in the outputs directory.")
#                 break
